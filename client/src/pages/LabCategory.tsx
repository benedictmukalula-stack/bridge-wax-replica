...
export default function LabCategory() {
  ...
  return <PageShell>
    ...
    <section className="section section-white"><div className="content-wrap">
      ...
      <div className="catalogue-grid">{catalogue.products.map((product) => <article className="catalogue-card" key={product.code}><div className="catalogue-card-media"><img src={product.image} alt={`${product.name} reference image`} loading="eager" decoding="sync" /></div><div className="catalogue-card-body"><span className="catalogue-code">{product.code}</span><h2>{product.name}</h2><p>{product.description}</p><AddToQuoteButton product={{ ...product, categorySlug: catalogue.slug, categoryTitle: catalogue.title }} /></div></article>)}</div>
    </div></section>
    <CatalogueBackToTop />
  </PageShell>;
}
