import { ArrowRight, ChevronRight, Search, ShieldCheck, SlidersHorizontal, X } from "lucide-react";
...
import { PPE_CATEGORY_GROUPS, PPE_CATALOGUE, PPE_PRODUCT_FAMILIES } from "../lib/ppeCatalogue";
...
const SOLUTION_GROUPS = [
  ...
  { title: "Industrial Safety", href: "/products/safety-ppe", text: "Safety & PPE product families for workwear, site protection, emergency readiness, and specialist industrial tasks." },
...
export default function Products() {
  ...
  return <PageShell>
    ...
    <section className="section section-white product-explorer-section"><div className="content-wrap"><div className="section-heading products-category-heading"><span className="eyebrow">Structured for procurement</span><h2>Browse the catalogue your way.</h2><p>Existing industrial product records remain derived from the live Bridge Wax catalogue. Safety &amp; PPE is a separate quotation-led discovery area, ready for approved manufacturer information.</p></div><div className="catalogue-category-explorer">{categories.map((item) => <article className="catalogue-category-card" key={item.slug}><div className="catalogue-category-image"><img src={item.image} alt={item.title} loading="lazy" decoding="async" /></div><div className="catalogue-category-copy"><span className="catalogue-category-count">{item.productCount} products</span><h3>{item.title}</h3><p>{item.summary}</p><div className="catalogue-category-ranges">{item.ranges.map((itemRange) => <span key={itemRange.slug}>{itemRange.title}</span>)}</div><Link href={`/products/${item.slug}`} className="catalogue-product-view">Explore Category <ArrowRight size={15} /></Link></div></article>)}<article className="catalogue-category-card catalogue-category-card-ppe"><div className="catalogue-category-image catalogue-ppe-symbol"><ShieldCheck size={44} aria-hidden="true" /></div><div className="catalogue-category-copy"><span className="catalogue-category-count">{PPE_PRODUCT_FAMILIES.length} product families</span><h3>{PPE_CATALOGUE.title}</h3><p>{PPE_CATALOGUE.summary}</p><div className="catalogue-category-ranges">{PPE_CATEGORY_GROUPS.slice(0, 3).map((group) => <span key={group.slug}>{group.title}</span>)}</div><Link href="/products/safety-ppe" className="catalogue-product-view">Explore Safety &amp; PPE <ArrowRight size={15} /></Link></div></article></div></div></section>
